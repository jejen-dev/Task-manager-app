    ...
    return (
        <div className={`${showDetails ? 'w-72' : 'w-auto'} bg-dark-bg dark:bg-dark-bg rounded-xl flex flex-col p-4 h-full transition-all duration-300`}>
            <div className="flex flex-col h-full">
                ...
                {!showDetails ? (
                    // MODE RINGKAS (tidak ada perubahan)
                    <>
                        <div className="flex flex-col items-center gap-3 overflow-y-auto hide-scrollbar">
                            {Object.values(boards).map((board) => (
                                <button
                                    ...
                                    className={`w-10 h-10 rounded-full transition-opacity ${activeBoardId === board.id ? 'opacity-100' : 'opacity-70 hover:opacity-100'
                                        }`}
                                >
                        ...
                        <button onClick={handleAddNewBoard} className="flex justify-center my-4 w-full">
                    ...
                    // MODE DETAIL
                    // !!! PERUBAHAN: Container scroll dengan posisi relatif agar sticky bekerja
                    <div className="flex-1 overflow-y-auto hide-scrollbar mb-4 relative">
                        <BoardList />
                        {/* !!! Tombol Add new board sticky di bagian bawah container */}
                        <div className="sticky bottom-0 bg-dark-bg dark:bg-dark-bg pt-2">
                            <button
                                onClick={handleAddNewBoard}
                                className="flex items-center gap-2 text-white bg-[#3A3E44] p-2 rounded-lg mt-4 w-full justify-center"
                            >
