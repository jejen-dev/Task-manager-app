            ...
            <Draggable draggableId={task.id} index={index}>
                {(provided, snapshot) => (
                    <div
                        ...
                        className={`bg-white dark:bg-[#191B1F] rounded-lg p-3 mb-3 shadow-sm cursor-pointer hover:shadow-md transition-shadow ${snapshot.isDragging ? 'opacity-50' : ''}`}
                    >
                        {task.coverImage && (
                            <img src={task.coverImage} alt="Task cover" className="w-full h-32 object-cover rounded-md mb-2" />
                        )}

                        <h4 className="text-sm font-medium text-black dark:text-white mb-2">{task.name}</h4>
